...
def new_topic(request):
	if request.method != 'POST':
		...
		if form.is_valid():
			form.save()
			return HttpResponseRedirect(reverse('my_logs:topics'))
...
def new_entry(request, topic_id):
	...
	if request.method != 'POST':
		...
		if form.is_valid():
			...
			new_entry.save()
			return HttpResponseRedirect(reverse('my_logs:topic', args=[topic_id]))
...
def edit_entry(request, entry_id):
	...
	if request.method != 'POST':
		...
		if form.is_valid():
			form.save()
			return HttpResponseRedirect(reverse('my_logs:topic', args = [topic.id]))
